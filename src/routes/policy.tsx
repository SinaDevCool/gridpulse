    ...
    return (
      <div className="min-h-screen bg-background text-foreground">
        ...
        <main className="mx-auto max-w-[1100px] px-4 py-12 lg:px-8">
          ...
          <p className="mt-3 max-w-2xl text-sm text-muted-foreground">FERC orders, EU directives, ISO market reforms, and incentive programs — tracked in one timeline.</p>
          <div className="mt-8 divide-y divide-border/50">
            {list.length === 0 ? <div className="py-12 text-center text-muted-foreground">No policy stories yet.</div> : list.map((a) => <ArticleRow key={a.id} article={a} />)}
          </div>
        </main>
        <SiteFooter />
      </div>
