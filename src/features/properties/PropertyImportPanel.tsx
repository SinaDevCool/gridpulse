...
function downloadTemplate() {
  const url = URL.createObjectURL(new Blob([propertyImportTemplateCsv()], { type: "text/csv;charset=utf-8" }));
...
export function PropertyImportPanel({ onImported }: { onImported: () => void }) {
  ...
  async function selectFile(file: File | undefined) {
    ...
    try {
      ...
      setRows([]);
      setError(reason instanceof Error ? reason.message : "The file could not be read. Check its format and try again.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <section className="workspace-card property-import-panel" aria-labelledby="property-import-title">
      ...
      {busy && !rows.length ? <p role="status" aria-live="polite">Reading portfolio…</p> : null}
        ...
        <>
          ...
          <div className="portfolio-table-wrap">
            <table className="decision-table property-import-preview">
              ...
              <thead><tr><th>Row</th><th>Property</th><th>External ID</th><th>Location</th><th>Required MW</th><th>Validation</th></tr></thead>
              <tbody>
                {rows.map((row) => (
                  <tr key={`${row.value.sourceRow}-${row.value.externalPropertyId ?? row.value.propertyName}`}>
                    <td>{row.value.sourceRow}</td>
                    <td>{row.value.propertyName || "Missing"}</td>
                    <td>{row.value.externalPropertyId ?? "—"}</td>
                    <td>{row.value.latitude ?? "Unknown"}, {row.value.longitude ?? "Unknown"}</td>
                    <td>{row.value.requiredTotalSiteLoadMw ?? "Unknown"}</td>
                    <td>{row.errors.length ? <ul>{row.errors.map((message) => <li key={message}>{message}</li>)}</ul> : <span className="status-badge is-ready">Ready</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="property-import-actions">
            <button type="button" className="secondary-button" onClick={() => { setRows([]); setFileName(""); }} disabled={busy}>Clear Preview</button>
            <button type="button" className="primary-button" onClick={() => void commit()} disabled={busy || invalid > 0}>
              {busy ? <LoaderCircle className="spin" aria-hidden="true" /> : <Upload aria-hidden="true" />}
              {busy ? "Importing…" : `Import ${rows.length} ${rows.length === 1 ? "Property" : "Properties"}`}
            </button>
          </div>
        </>
      ) : null}
