...
function transactionDone(transaction: IDBTransaction): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    transaction.onabort = () => reject(transaction.error ?? new Error("The browser transaction was cancelled."));
  });
}
...
export function subscribeAnonymousWorkspace(listener: () => void) {
  ...
  return () => { window.removeEventListener(CHANGE_EVENT, listener); channel?.close(); };
}
...
export async function getAnonymousProperty(id: string): Promise<AnonymousProperty | null> {
  const db = await openDatabase();
  try {
    const value = await requestValue(db.transaction(PROPERTIES, "readonly").objectStore(PROPERTIES).get(id));
    return isAnonymousProperty(value) ? value : null;
  } finally { db.close(); }
}
...
export async function saveAnonymousProperty(property: AnonymousProperty): Promise<void> {
  ...
  try {
    ...
    transaction.objectStore(VERSIONS).put({ id: `${property.id}:${property.updatedAt}`, propertyId: property.id, snapshot: property, createdAt: property.updatedAt });
...
export async function importAnonymousProperties(
  ...
  let skipped = 0;
  try {
    const existing = await requestValue(db.transaction(PROPERTIES, "readonly").objectStore(PROPERTIES).getAll()) as AnonymousProperty[];
    const byExternal = new Map(existing.filter((item) => item.externalPropertyId).map((item) => [item.externalPropertyId!.toLocaleLowerCase(), item]));
    const transaction = db.transaction([PROPERTIES, VERSIONS], "readwrite");
    for (const property of properties) {
      const match = property.externalPropertyId ? byExternal.get(property.externalPropertyId.toLocaleLowerCase()) : undefined;
      if (match && conflict === "skip") { skipped += 1; continue; }
      const next = match ? { ...property, id: match.id, createdAt: match.createdAt } : property;
      transaction.objectStore(PROPERTIES).put(next);
      transaction.objectStore(VERSIONS).put({ id: `${next.id}:${next.updatedAt}`, propertyId: next.id, snapshot: next, createdAt: next.updatedAt });
      imported += 1;
    }
    await transactionDone(transaction);
  } finally { db.close(); }
...
export async function deleteAnonymousProperty(id: string): Promise<void> {
  ...
  try {
    ...
    const versions = await requestValue(transaction.objectStore(VERSIONS).getAll()) as Array<{ id: string; propertyId: string }>;
    versions.filter((item) => item.propertyId === id).forEach((item) => transaction.objectStore(VERSIONS).delete(item.id));
    await transactionDone(transaction);
  } finally { db.close(); }
  notify();
}
...
export async function clearAnonymousWorkspace(): Promise<void> {
  ...
  try {
    const transaction = db.transaction([PROPERTIES, VERSIONS, METADATA], "readwrite");
    transaction.objectStore(PROPERTIES).clear(); transaction.objectStore(VERSIONS).clear(); transaction.objectStore(METADATA).clear();
    await transactionDone(transaction);
  } finally { db.close(); }
  notify();
}
...
export async function exportAnonymousWorkspace(): Promise<AnonymousWorkspaceBackup> {
  return { product: "gridpulse-anonymous-workspace", schemaVersion: ANONYMOUS_WORKSPACE_SCHEMA_VERSION, exportedAt: new Date().toISOString(), properties: await listAnonymousProperties() };
}

export async function restoreAnonymousWorkspace(value: unknown, conflict: "skip" | "replace" = "replace") {
  ...
  if (backup.product !== "gridpulse-anonymous-workspace" || !Array.isArray(backup.properties) || !backup.properties.every(isAnonymousProperty))
    throw new Error("The file is not a valid GridPulse workspace backup.");
  return importAnonymousProperties(backup.properties.map((property) => ({ ...property, source: "workspace_restore" })), conflict);
}
