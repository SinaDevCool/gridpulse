...
as $$
  with bounds as (
    ...
    where a.geometry is not null and a.location_precision in ('surveyed','mapped')
      ...
      and (a.source_artifact_id is null or exists (select 1 from public.grid_source_artifacts f where f.id=a.source_artifact_id and f.status='active'))
    order by coalesce(a.net_capacity_mw,0) desc,a.source_record_id
    limit (case when z<9 then 500 when z=9 then 10000 else 20000 end)
  ), overview as (
    select md5(kind||generation_group||extensions.st_astext(extensions.st_snaptogrid(geometry,case when z<=6 then .35 else .12 end))) id,
    ...
    select r.id,r.kind,r.name,r.operator,r.status,r.technology,r.generation_group,r.asset_count,r.known_mw_count,r.registered_mw,r.net_capacity_mw,r.storage_energy_mwh,r.source_url,
