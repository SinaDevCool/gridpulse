...
returns bytea language sql stable security definer set search_path = '' set statement_timeout = '24s'
as $$
  with bounds as (
    ...
    select a.source_record_id id,
      ...
      case when a.asset_type='storage' then 'storage'
      ...
      a.storage_energy_mwh::double precision storage_energy_mwh,s.source_url,a.geometry
    ...
    where a.geometry is not null and a.location_precision in ('surveyed','mapped')
      ...
      and (a.source_artifact_id is null or exists (select 1 from public.grid_source_artifacts r where r.id=a.source_artifact_id and r.status='active'))
    order by coalesce(a.net_capacity_mw,0) desc,a.source_record_id
    limit (case when z<9 then 5000 when z<11 then 10000 else 20000 end)
  ), overview as (
    select md5(kind||generation_group||extensions.st_astext(extensions.st_snaptogrid(geometry,
      case when z<=6 then .35 when z<=8 then .12 else .035 end))) id,
      kind,generation_group,count(*)::integer asset_count,count(net_capacity_mw)::integer known_mw_count,
      sum(net_capacity_mw)::double precision registered_mw,
      extensions.st_centroid(extensions.st_collect(geometry)) geometry
    from assets where z<11 group by kind,generation_group,
      extensions.st_snaptogrid(geometry,case when z<=6 then .35 when z<=8 then .12 else .035 end)
  ), tile_rows as (
    select id,kind,generation_group,asset_count,known_mw_count,registered_mw,null::text name,
      null::text operator,null::text status,null::text technology,null::double precision net_capacity_mw,
      null::double precision storage_energy_mwh,null::text source_url,geometry from overview
    union all select id,kind,generation_group,1,case when net_capacity_mw is null then 0 else 1 end,
      net_capacity_mw,name,operator,status,technology,net_capacity_mw,storage_energy_mwh,source_url,geometry
      from assets where z>=11
  ), mvt as (
    select r.id,r.kind,r.name,r.operator,r.status,r.technology,r.generation_group,r.asset_count,
      r.known_mw_count,r.registered_mw,r.net_capacity_mw,r.storage_energy_mwh,r.source_url,
      extensions.st_asmvtgeom(extensions.st_transform(r.geometry,3857),b.geom_3857,4096,64,true) geom
    from tile_rows r cross join bounds b order by coalesce(r.registered_mw,0) desc,r.id
    limit (case when z<=8 then 5000 when z<11 then 10000 else 20000 end)
